...
async def perform_ocr(file: UploadFile) -> str:
    ...
    if file.content_type not in allowed_formats:
        raise HTTPException(status_code=400, detail="File type not supported. Supported types are: PNG, JPEG, PDF")
    ...
    if response.status_code == 200:
        ...
        raise HTTPException(status_code=response.status_code, detail=f"OCR failed: {response.text}")
